...
| .NET apphost used by `PocketBridge.App.exe` | Compatible .NET 8 SDK selected by `global.json`; verified RC workflow used SDK 8.0.424 | [.NET runtime](https://github.com/dotnet/runtime) | MIT; Copyright (c) .NET Foundation and Contributors; additional upstream notices | Yes, apphost only | Include the runtime MIT license and applicable third-party notices. The .NET 8 Desktop Runtime itself is required separately and is not included in the framework-dependent ZIP. Re-audit the exact runtime/apphost notices when changing the SDK line or distribution model. |
