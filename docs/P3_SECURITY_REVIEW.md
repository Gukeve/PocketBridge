...
| Replay | Every control message carries a strictly increasing positive sequence scoped to its active session; stale/duplicate messages close the socket. | Automated live-WebSocket concurrency tests remain required. |
...
| Sensitive audit data | Atomically persisted, bounded 500 records; actor, alias, action and result only. Tokens, clipboard/file contents and full serial are excluded. Corrupt files are quarantined. | Audit viewer UI remains incomplete. |

## Security disposition

The code now enforces active-session, expiry, permission, exact-Origin and anti-replay checks for every typed control message; revoke aborts its control socket immediately. Security status remains **PARTIAL** until adversarial live-WebSocket tests, audit UI review and hardware LAN acceptance pass. Uploads, remote clipboard, arbitrary commands and browser-provided serials remain unsupported.
